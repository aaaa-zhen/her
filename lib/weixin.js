...
class WeixinService extends EventEmitter {
  ...
  async _chat(conversationId, text, images = []) {
    ...
    // Global timeout: if _chat takes longer than 90s, abort and reset
    const chatTimeout = setTimeout(() => {
      console.error("[WeChat] _chat timed out (90s), resetting session");
      session.history = [];
      this._saveSession(conversationId, session);
    }, 90000);
    try {
      return await this._chatInner(conversationId, session, text, images);
    } catch (err) {
      console.error("[WeChat] _chat fatal error, resetting session:", err.message);
      session.history = [];
      this._saveSession(conversationId, session);
      return { text: `出错了，已重置对话: ${err.message}` };
    } finally {
      clearTimeout(chatTimeout);
    }
  }

  async _chatInner(conversationId, session, text, images = []) {
    ...
    while (response && response.stop_reason === "tool_use" && maxIterations-- > 0) {
      ...
      try {
        ...
        console.error("[WeChat] Tool loop AI error, resetting history:", toolErr.message);
        // History is corrupted — keep only the original user message and break
        session.history = [session.history.find(m => m.role === "user" && typeof m.content === "string") || session.history[0]];
        response = null;
        break;
      }
    }
