...
async function executeTool(block, ws, activeProcesses, schedulerCtx) {
  ...
  // ===== Search Web =====
    ...
    try {
      ...
      // Try DDG Lite first (more reliable, less likely to be blocked)
      const liteCmd = `curl -sL "https://lite.duckduckgo.com/lite/?q=${encoded}" -H "User-Agent: Mozilla/5.0" | head -c 200000`;
      const liteHtml = await execAsync(liteCmd, { timeout: 15000 });
      const results = [];

      // DDG Lite format: result links have rel="nofollow" and class="result-link"
      const liteRegex = /<a rel="nofollow" href="([^"]*)"[^>]*class=['"]result-link['"][^>]*>([\s\S]*?)<\/a>/g;
      const snippetRegex = /<td class="result-snippet">([\s\S]*?)<\/td>/g;
      const links = [];
      let m;
      while ((m = liteRegex.exec(liteHtml)) !== null) {
        let href = m[1];
      ...
      // Collect snippets
      const snippets = [];
      while ((m = snippetRegex.exec(liteHtml)) !== null) {
      ...
      // Fallback to DDG HTML if lite returned nothing
      if (results.length === 0) {
