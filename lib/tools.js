...
function getToolDefinitions(model) {
  ...
  const allTools = [
    ...
    {
      name: "fan_control",
      description: "Control the USB fan connected to the computer. Use this when the user asks to turn on/off the fan, check fan status, or anything related to the fan (风扇).",
      input_schema: {
        type: "object",
        properties: {
          action: { type: "string", enum: ["on", "off", "status"], description: "on=turn on, off=turn off, status=check current state" },
        },
        required: ["action"],
      },
    },
...
async function executeTool(block, ws, activeProcesses, schedulerCtx) {
  ...
  // ===== Fan Control =====
  if (block.name === "fan_control") {
    const { action } = block.input;
    const FAN_API = process.env.FAN_API_URL || "http://localhost:9999";
    try {
      const endpoint = action === "on" ? "/api/on" : action === "off" ? "/api/off" : "/api/status";
      const resp = await fetch(`${FAN_API}${endpoint}`, { signal: AbortSignal.timeout(5000) });
      const data = await resp.json();
      const stateText = data.on ? "开启中 (Running)" : "已关闭 (Off)";
      const actionText = action === "on" ? "已开启风扇" : action === "off" ? "已关闭风扇" : "风扇状态";
      ws.send(JSON.stringify({ type: "command", command: `fan: ${action}` }));
      ws.send(JSON.stringify({ type: "command_output", output: `${actionText}: ${stateText}` }));
      return { type: "tool_result", tool_use_id: block.id, content: `${actionText}: ${stateText}` };
    } catch (err) {
      return { type: "tool_result", tool_use_id: block.id, content: `Fan control error: ${err.message}. Is the fan server running at ${FAN_API}?`, is_error: true };
    }
  }
